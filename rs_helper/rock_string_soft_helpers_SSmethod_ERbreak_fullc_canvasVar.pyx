...
from rs_helper.operations.operations import operations_fullc_canvasVar as operations_fullc
...
class Soft_and_Filament(object):
    ...
    """
    def __init__(self, dim = 2, Nc = 10, Nv = 20, Nf = 0, Np = 10, 
                Rc: cython.double = 1.0, bidisperse: cython.double = 1.4, mLf: cython.double = 6.4, v_char: cython.double = 0.01,
                 Ca: cython.double = 0.039, St: cython.double = 1., Re_R: cython.double = 0.04, Stk: cython.double = 2.4E-3, 
                 K1: cython.double = 1.2E-3, K2_pos: cython.double = 17.6, K2_neg: cython.double = 17.6, K3: cython.double = 0.39, 
                 K4: cython.double = 5.1E-8, kr_b: cython.double = 1., kr_bf: cython.double = 1., rho: cython.double = 1000.,
                 seed: cython.int = 0, random_init = True, periodic_bc = False, full_repulsion = True, 
                 Aspect_ratio: cython.double = 10.0, Length_radius_ratio: cython.double = 100.0, canvas_initial_xy = None, 
                 viscous_ratio: cython.double = 50.0):
        """
        ...
        self.viscous_ratio = viscous_ratio
        ...
        self.eta_w = self.eta/self.viscous_ratio
        ...
        self.fullc_op = operations_fullc.full_rhs_cython(self.Nc, self.Nv, self.Nf, self.Np, self.canvas.n_corners, self.kr, self.kr_b, 
            self.ka, self.gamma, self.kl_c_pos*self.Nv, self.kl_c_neg*self.Nv, self.kl, self.filament_bending_prefactor, self.kr_bf, 
            self.six_pi_eta_r, self.delt, self.dLf, self.eta, self.eta_w, self.boundary_forces_external, self.Lx0, self.Ly0)
        self.r_expanded_0[(self.Nc*self.Nv*self.dim + 2*self.Nf*self.Np)] = self.half_box_size_x
        self.r_expanded_0[(self.Nc*self.Nv*self.dim + 2*self.Nf*self.Np+1)] = self.half_box_size_y
        self.r_expanded[(self.Nc*self.Nv*self.dim + 2*self.Nf*self.Np)] = self.half_box_size_x
        self.r_expanded[(self.Nc*self.Nv*self.dim + 2*self.Nf*self.Np+1)] = self.half_box_size_y
    ...
    def allocate_arrays(self):
        ...
        self.r_expanded = np.zeros(self.Nc*self.Nv*self.dim + self.dim*self.Nf*self.Np + 2, 
            dtype = np.double)
        ...
        self.r_expanded_0 = np.zeros(self.Nc*self.Nv*self.dim+self.Nc+self.dim*self.Nf*self.Np + 2, 
            dtype = np.double)
    ...
    def initialize_colloid_location(self):
        ...
        self.six_pi_eta_r = self.eta*self.Rc*(PI/self.Nv)
    ...
    def change_r_expanded(self, r_expanded):
        ...
        self.f_y_array = np.reshape(
            ...
            (self.Nf, self.Np), order = 'C')
        self.half_box_size_x =  r_expanded[(self.dim*self.Nc*self.Nv+2*self.Nf*self.Np)]
        self.half_box_size_y =  r_expanded[(self.dim*self.Nc*self.Nv+2*self.Nf*self.Np+1)]
        self.change_canvas(np.array([[-self.half_box_size_x, -self.half_box_size_y], 
                [self.half_box_size_x, -self.half_box_size_y], 
                [self.half_box_size_x, self.half_box_size_y], 
                [-self.half_box_size_x, self.half_box_size_y]]))
    ...
    def _Fr(self):
        ...
        if self.Nf == 0:
            ...
            if self.full_repulsion:
                ...
                #f_collide, p_collide = divmod(fp_all[:, None], self.Np)
                f_collide, p_collide = divmod(fp_all, self.Np)"""
                ...
                """Frx, Fry, Fvx_int, Fvy_int = self.op.interact_with_vacuole(xy_x, xy_y, xy2_x, xy2_y, xs, ys, self.delt/2, n_pair)

                np.add.at(self.Frx, [f_collide, p_collide], Frx.T)
                np.add.at(self.Fry, [f_collide, p_collide], Fry.T)
                vertex_all = np.tile(np.arange(self.Nv), n_pair)
                cc_all = np.tile(c_all[:, None], self.Nv).flatten()
                np.add.at(self.Fv, [cc_all, vertex_all, 0], Fvx_int.T.flatten())
                np.add.at(self.Fv, [cc_all, vertex_all, 1], Fvy_int.T.flatten())"""
    ...
    def _KE(self, r_expanded, t):
        ...
        drEdt = np.array(self.drEdt[0:self.dim*self.Nc*self.Nv+self.dim*self.Nf*self.Np])
        KE = 1/2*np.sum((drEdt)**2)/len(drEdt)
        KE_velocity_ave = 1/2*np.sum((drEdt[0:self.dim*self.Nc*self.Nv])**2)/self.Nc
    ...
    def rhs_cython(self, r_expanded, t):
        ...
        if self.periodic_bc:
            ...
            self.Fvx, self.Fvy, self.Fax, self.Fay, self.Fbx, self.Fby, self.Flcx, self.Flcy, self.Fgx, self.Fgy, self.Flx, self.Fly, self.Frx, self.Fry, self.Fbfx, self.Fbfy, self.fx_on_wall, self.fy_on_wall, self.drEdt = self.fullc_op.rhs_cython(
                self.r_matrix, self.rx, self.ry, self.f_x_array, self.f_y_array, self.f_x_array.flatten(), self.f_y_array.flatten(),
                self.connection_table, self.connection_vertex, self.axis_1_plus_1, self.axis_1_minus_1, self.axis_1_plus_2, 
                self.area0_arr.flatten(), self.l0_arr.flatten(), self.canvas.xy, self.canvas.xy2, self.canvas.a.flatten(), 
                self.canvas.b.flatten(), self.canvas.c.flatten(), self.canvas.M.flatten(), 
                self.canvas.center_sign_to_border.flatten().astype(np.int32), self.ER_intact, self.vacuole_bending_prefactor.flatten(), 
                self.half_box_size_x, self.half_box_size_y)
            ...
            if(file[-4:] == 'hdf5'):
                ...
                with h5py.File(file, "r") as f:
                    if('simulation data' in f.keys()): # Load the simulation data (newer method)
                        ...
                        self.viscous_ratio = dset.attrs['viscous_ratio']
    ...
    def save_data(self):
        ...
        with h5py.File(os.path.join(self.saveFolder, self.saveFile), "w") as f:
            ...
            dset.attrs['viscous_ratio'] = self.viscous_ratio
