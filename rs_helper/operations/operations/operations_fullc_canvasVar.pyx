...
cdef class full_rhs_cython:
    def __init__(self, long Nc, long Nv, long Nf, long Np, long n_corners, double kr, double kr_b, double ka, double gamma, double kl_c_pos_Nv, double kl_c_neg_Nv, double kl, double filament_bending_prefactor, double kr_bf, double six_pi_eta_r, double delt, double dLf, double eta, double eta_w, double boundary_forces_external, double Lx0, double Ly0):
        self.Nc = Nc
        self.Nv = Nv
        self.Nf = Nf
        self.Np = Np
        ...
        self.eta = eta
        self.eta_w = eta_w
        self.boundary_forces_external = boundary_forces_external
        self.Lx0 = Lx0
        self.Ly0 = Ly0
    ...
    cpdef find_within_canvas_and_wall_force(self, double[:] xs, double[:] ys, double delta, long n_site, double[:,:] xy, double[:,:] xy2, double[:] a, double[:] b, double[:] c, double[:] M, int[:] center_sign_to_border):
        ...
        cdef double[:] fx_on_wall = np.zeros(self.n_corners, dtype = np.double)
        cdef double[:] fy_on_wall = np.zeros(self.n_corners, dtype = np.double)
        for ii in range(n_site):
            ...
            if roundc((abs(ang) - 2*PI)*10**9) == 0:
                ...
                for cc in range(self.n_corners):
                    ...
                    if delta_m_dist > 0:
                        fx = (xs[ii] - xp)/(dist_perp+1E-10)*delta_m_dist
                        fy = (ys[ii] - yp)/(dist_perp+1E-10)*delta_m_dist
                        f_x[ii] += fx
                        f_y[ii] += fy
                        fx_on_wall[cc] -= fx
                        fy_on_wall[cc] -= fy
                ...
                for cc in range(self.n_corners):
                    ...
                    if sign != center_sign_to_border[cc]:
                        dist_perp = abs(dist_perp)
                        fx = (xs[ii] - xp)/(dist_perp+1E-10)*(dist_perp+delta)
                        fy = (ys[ii] - yp)/(dist_perp+1E-10)*(dist_perp+delta)
                        f_x[ii] -= fx
                        f_y[ii] -= fy
                        fx_on_wall[cc] += fx
                        fy_on_wall[cc] += fy
        return f_x, f_y, fx_on_wall, fy_on_wall
    ...
    cpdef rhs_cython(self, double[:,:,:] r_matrix, double[:] rx, double[:] ry, double[:,:] f_x_array, double[:,:] f_y_array
        ...
        int[:] center_sign_to_border, long[:,:] ER_intact, double[:] vacuole_bending_prefactor, double hLx, double hLy):
        ...
        cdef double[:] fx
        cdef double[:] fy
        cdef double[:] fx_on_wall
        cdef double[:] fy_on_wall
        cdef double[:] drEdt = np.zeros((2*self.Nc*self.Nv+2*self.Nf*self.Np+2), dtype = np.double)
        ...
        cdef double fx_on_wall_net = 0.
        cdef double dhLx, dhLy
        ...
        fx, fy, fx_on_wall, fy_on_wall = self.find_within_canvas_and_wall_force(rx, ry, self.delt/2, self.Nc*self.Nv, xy, xy2, 
                a, b, c, M, center_sign_to_border)
        for ww in range(self.n_corners):
            fx_on_wall_net += abs(fx_on_wall[ww])
        fx_on_wall_net *= self.kr*self.kr_b/2
        if fx_on_wall_net >= self.boundary_forces_external:
            pass
        else:
            dhLx = -(self.boundary_forces_external - fx_on_wall_net)/(self.eta_w*6*PI*self.Ly0)
            dhLy = -dhLx/hLx*hLy
            drEdt[2*self.Nc*self.Nv+2*self.Np*self.Nf] = dhLx
            drEdt[2*self.Nc*self.Nv+2*self.Np*self.Nf+1] = dhLy
        ...
        if (self.kr_bf > 0) and (self.Nf > 0):
            ffx, ffy, _, _ = self.find_within_canvas_and_wall_force(
        ...
        return Fvx, Fvy, Fax, Fay, Fbx, Fby, Flcx, Flcy, Fgx, Fgy, Flx, Fly, Frx, Fry, Fbfx, Fbfy, fx_on_wall, fy_on_wall, drEdt
