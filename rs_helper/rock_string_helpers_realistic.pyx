...
def is_on_segment_ez_vectorized(xp, yp, xy, xy2):
    """
    This function only check if the point is between the 2 end of the edges. It is thus assuming that
    the point is guaranteed to be on the extended line.
    """
    xy_min = np.minimum(xy, xy2)
    xy_max = np.maximum(xy, xy2)
    is_on_seg_result = (np.less_equal(xp, xy_max[:,0:1])*np.greater_equal(xp, xy_min[:,0:1])
        *np.less_equal(yp, xy_max[:,1:2])*np.greater_equal(yp, xy_min[:,1:2]))
    return is_on_seg_result
...
class PolygonClass(object):
    ...
    def find_wall_potential_vectorized(self, xs, ys, delta):
        n_site = len(xs)
        within_canvas = self.point_is_within_canvas(xs, ys)

        #xy2 = np.roll(self.xy, shift = 1, axis = 0)
        ...
        #is_on_seg_result = is_on_segment_ez_vectorized(xp, yp, self.xy, xy2)

        #min_dist = dist_perp*is_on_seg_result + np.minimum(np.sqrt((self.xy[:,0:1] - xs.T)**2 + (self.xy[:,1:2] - ys.T)**2), 
        #    np.sqrt((xy2[:,0:1] - xs.T)**2 + (xy2[:,1:2] - ys.T)**2))*(1-is_on_seg_result)
        min_dist = dist_perp

        dist_perp = min_dist*within_canvas.T - min_dist*(1-within_canvas.T)
        TF_delta = (dist_perp < delta)

        U_all = (1/2*(delta - dist_perp)**2*TF_delta).T
        U_sum = np.sum(U_all, axis = 1, keepdims = True)
        min_dist_loc = np.argmin(np.abs(min_dist.T), axis = 1)
        U_final = U_sum*within_canvas + U_all[range(n_site), min_dist_loc].reshape((n_site, 1))*(1-within_canvas)

        return U_final, U_all, within_canvas, min_dist_loc

    """def find_wall_potential(self, xs, ys, delta):
        '''
        Find the repulsive potential between particles with the wall
        '''
...
class Colloid_and_Filament(object):
    ...
    """
    def __init__(self, dim = 2, Nc = 10, Nf = 0, Np = 10, Rc = 1, bidisperse = 1.4, mLf = 6.4, v_char = 0.01,
                 Ca = 0.039, St = 1, Re_R = 0.04, Stk = 2.4E-3, 
                 K2 = 8.9, K4 = 5.1E-8, kr_b = 1, kr_bf = 1, rho = 1000,
        ...
        K2 = kr/kl, positive indicates l_{m,l} > l0, negative indicates l_{m,l} < l0
        K4 = kbf/(kl*R^4)
        ...
        kr_b = relative repulsive strength from the boundary, for the filament
        kl = spring constant of filaments
        kbf = bending coefficient of filaments
        ...
        self.kr_bf = kr_bf # should set to 1 during initialization, but turn to 0 when simulation
        self.rho = rho
        ...
        self.kl = self.eta*self.v_char/self.Ca # matching capillary number
        self.kr = self.K2*self.kl # matching relative strength between repulsion and membrane stretching
        self.kbf = self.K4*self.kl*self.Rc**4 # matching relative strength between bending and expansion modulus
    ...
    def plot_system(self, plot_energy = False, colormap = cm.viridis, alpha_colloid = 1, alpha_fil = 1, 
        string_color = None, vac_color = 'silver', edge_color = 'black', lw = 2):
        """
    ...
    def _Fbf(self):
        ...
        # compute forces
        self.Fbfx[:, 0] = self.filament_bending_prefactor*(self.dx[:,0] - self.dx[:,1])
        self.Fbfy[:, 0] = self.filament_bending_prefactor*(self.dy[:,0] - self.dy[:,1])
        self.Fbfx[:, 1] = self.filament_bending_prefactor*(-2*self.dx[:,0] + 3*self.dx[:,1] - self.dx[:,2])
        self.Fbfy[:, 1] = self.filament_bending_prefactor*(-2*self.dy[:,0] + 3*self.dy[:,1] - self.dy[:,2])
        self.Fbfx[:, 2:self.Np-2] = self.filament_bending_prefactor*(
            3*self.dx[:, 2:self.Np-2] - 3*self.dx[:, 1:self.Np-3] + self.dx[:, 0:self.Np-4] - self.dx[:, 3:self.Np-1])
        self.Fbfy[:, 2:self.Np-2] = self.filament_bending_prefactor*(
            3*self.dy[:, 2:self.Np-2] - 3*self.dy[:, 1:self.Np-3] + self.dy[:, 0:self.Np-4] - self.dy[:, 3:self.Np-1])
        self.Fbfx[:, self.Np-1] = self.filament_bending_prefactor*(self.dx[:,self.Np-3] - self.dx[:,self.Np-2])
        self.Fbfy[:, self.Np-1] = self.filament_bending_prefactor*(self.dy[:,self.Np-3] - self.dy[:,self.Np-2])
        self.Fbfx[:, self.Np-2] = self.filament_bending_prefactor*(self.dx[:,self.Np-4] - 3*self.dx[:,self.Np-3] + 2*self.dx[:,self.Np-2])
        self.Fbfy[:, self.Np-2] = self.filament_bending_prefactor*(self.dy[:,self.Np-4] - 3*self.dy[:,self.Np-3] + 2*self.dy[:,self.Np-2])
    ...
    def _Fr(self):
        ...
        if self.full_repulsion:
            ...
            #self.Frx[:, 1:self.Np-1] += self.kr/self.Np*(self.r_vac0 - self.dr_to_vac0)*(
            #    self.r_vac0 > self.dr_to_vac0)/self.dr_to_vac0*self.dx_to_vac0
            #self.Frx[:, 1:self.Np-1] += self.kr/self.Np*(self.r_vac1 - self.dr_to_vac1)*(
            #    self.r_vac1 > self.dr_to_vac1)/self.dr_to_vac1*self.dx_to_vac1
            #self.Fry[:, 1:self.Np-1] += self.kr/self.Np*(self.r_vac0 - self.dr_to_vac0)*(
            #    self.r_vac0 > self.dr_to_vac0)/self.dr_to_vac0*self.dy_to_vac0
            #self.Fry[:, 1:self.Np-1] += self.kr/self.Np*(self.r_vac1 - self.dr_to_vac1)*(
    ...
    def compute_potential_energy(self, r_expanded):
        ...
        if self.periodic_bc:
            ...
            U_total = U_rep_colloid + Ul + Ur + Urb + Ubf
            ...
            U_total = U_rep_colloid + Ul + Ur + Urb + Ubf + Urbf
            U_total_average = U_total/self.Nc
            U_colloid_average = (U_rep_colloid + Urb)/self.Nc
    ...
    def _Ubf(self):
        ...
        Ubf = self.filament_bending_prefactor/2*np.sum(
            (self.dx[:, 0:self.Np-2] - self.dx[:, 1:self.Np-1])**2 + 
            (self.dy[:, 0:self.Np-2] - self.dy[:, 1:self.Np-1])**2)
        return Ubf
    ...
    def _Ur(self):
        ...
        if self.full_repulsion:
            ...
            #Ur = 1/2*self.kr/self.Np*np.sum((self.radius[None, :, None] - self.dr_to_vac)**2*(self.radius[None, :, None] > self.dr_to_vac))
    ...
    def _Urb(self):
        ...
        if self.periodic_bc:
            ...
            Urb, _= self.canvas.find_wall_potential_vectorized_heuristic(
                self.r[0:self.Nc].reshape((self.Nc,1)), 
                self.r[self.Nc:].reshape((self.Nc,1)), 
                self.radius.reshape((1,self.Nc)))
        ...
        if self.periodic_bc:
            ...
            if self.kr_bf > 0:
                Urbf, _ = self.canvas.find_wall_potential_vectorized_heuristic(
                    ...
                    self.delta_string)
                #Urbf[0:self.Nf*self.Np:10] = 0
                #Urbf[self.Np-1:self.Nf*self.Np:10] = 0
                #Urbf = self.kr_bf*self.kr/self.Np*np.sum(Urbf)
    ...
    def rhs_cython(self, r_expanded, t):
        ...
        if self.periodic_bc:
            ...
            for i in range(self.Nf):
                self.F[0+self.connection_table[i, 0]] += self.Flx[i, 0] + self.Fbfx[i, 0]
                self.F[0+self.connection_table[i, 1]] += self.Flx[i, self.Np-1] + self.Fbfx[i, self.Np-1]
                self.F[self.Nc+self.connection_table[i, 0]] += self.Fly[i, 0] + self.Fbfy[i, 0]
                self.F[self.Nc+self.connection_table[i, 1]] += self.Fly[i, self.Np-1] + self.Fbfy[i, self.Np-1]
            ...
            self.Trq_temp0 = self.radius[self.connection_table[:, 0]]*(
                    -(self.Flx[:, 0] + self.Fbfx[:, 0])*np.sin(self.connection_theta[:, 0] + self.theta[self.connection_table[:, 0]]) 
                    +(self.Fly[:, 0] + self.Fbfy[:, 0])*np.cos(self.connection_theta[:, 0] + self.theta[self.connection_table[:, 0]]))
            self.Trq_temp1 = self.radius[self.connection_table[:, 1]]*(
                    -(self.Flx[:, self.Np-1] + self.Fbfx[:, self.Np-1])*np.sin(self.connection_theta[:, 1] + self.theta[self.connection_table[:, 1]]) 
                    +(self.Fly[:, self.Np-1] + self.Fbfy[:, self.Np-1])*np.cos(self.connection_theta[:, 1] + self.theta[self.connection_table[:, 1]]))
            for i in range(self.Nf):
            ...
            # Forces on vacuoles from boundary
            ...
            # Forces on the strings from the boundaries
            if self.kr_bf > 0:
                ffx, ffy = self.canvas.find_wall_force_vectorized_heuristic(
                    self.f_x_array.reshape((self.Nf*self.Np, 1)),
                    self.f_y_array.reshape((self.Nf*self.Np, 1)), self.delta_string)
                ffx[0:self.Nf*self.Np:10] = 0 # no boundary force on the connection site, because the vertex has experienced that already
                ffx[self.Np-1:self.Nf*self.Np:10] = 0
                ffy[0:self.Nf*self.Np:10] = 0
                ffy[self.Np-1:self.Nf*self.Np:10] = 0
            ...
            self.dfil_x_dt = ((self.Flx + self.Frx).flatten() + ffx)/(6*PI*self.eta*self.dLf)
            self.dfil_y_dt = ((self.Fly + self.Fry).flatten() + ffy)/(6*PI*self.eta*self.dLf)
